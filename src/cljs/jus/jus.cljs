...
(defn add-nova-naredba [& old-id]
	(let [data @nova-naredba-atom
				...
				id (or old-id (new-id))
				fields-data (merge naslov direktiva link file naredba napomena glasnik {:JUSId id :Locked 0})]
...
(defn edit-nova-naredba []
	(let [data @nova-naredba-atom
				...
				fields-data (merge naslov direktiva link file glasnik naredba napomena)]
...
(defn add-nova-naredba-event [naredba]
	(reset! nova-naredba-atom {:naslov nil :glasnik nil :direktiva nil :link nil :file nil :naredba naredba :napomena nil :X-CSRF-Token (h/get-value "__anti-forgery-token")})
...
(defn edit-nova-naredba-event [_ data]
	(reset! nova-naredba-atom {:jusid (:id data) :naslov (:naslov-full data) :glasnik (:glasnik data) :direktiva (:direktiva data)
														 :link  (:link-d data) :file (:link-n data) :naredba (:naredba data) :napomena (:napomena data) :X-CSRF-Token (h/get-value "__anti-forgery-token")})
...
(def col-widths-types-naredbe {:naslov    {:type :label :width "63%" :field "JUSopis" :action :click :function set-path :double-click reset-path :tooltip "Puni naslov: "}
...
(def col-widths-types-naredbe-stare {:naslov  {:type :label :width "65%" :field "JUSopis" :action :click :function set-path :double-click reset-path :tooltip "Puni naslov: "}
...
(defn rows-naredbe [data screen]
	(into {} (mapv (fn [x] (let [{:keys [JUSId JUSopis Glasnik Direktiva Link-n Link-d Naredba Locked Napomena]} x
...
(defn rows-naredbe-stare [data screen level] (into {} (mapv (fn [x] (let [{:keys [JUSId JUSopis Glasnik Link-n Link-d Naredba Locked Napomena]} x
																																					count-veza (count-veza JUSId)]
																																			(if (> Naredba 0)
																																				{JUSId {:id          JUSId
																																								:naslov-full JUSopis
