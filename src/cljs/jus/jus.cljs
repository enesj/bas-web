...
(defn lock-jus [id field data]
  (let [criteria {:JUSId id}
        data (if (= 0 data) 1 0)]
    (GET "/jus/update" {:params        {:filter criteria :field-data [{field data}] :like false}
                        :handler       (fn [x] (if (= x 1) (swap! jus-data update-in [:data] (fn [y] (setval [ALL #(= id (:JUSId %)) :Locked] data y)))))
                        :error-handler #(println "some error occured: " %)})))
...
(defn current-parent-child [& id]
  (let [local-state @table-state
        ...
        id (if id (first id) choice)
        parent (keyword (str (count path)))
        parent (parent path)
        child id]
    [parent child]))
...
(defn add-veza [& id]
  (let [id (if id (first id) (:choice @showing))
        new (first (filterv #(= id (:JUSId %)) (:data @jus-all-data)))
        [parent child] (current-parent-child id)]
    (do (GET "/jus/add-veza" {:params        {:parent parent :child child}
                          :handler       #(do (swap! showing assoc-in [:choice] nil) (init-veza [parent child]))
                          :error-handler #(do (println "some error occured: " %) )})
    (if-not (some #(= (:JUSId %) (:JUSId new)) (:data @jus-data))
      (swap! jus-data update-in [:data] #(merge % new)))
        (swap! count-veze update-in [parent :total] inc)
        (swap! count-veze update-in [parent :childs] #(conj % child)))
        ))

(defn delete-veza []
  (let [{:keys [level child]} (:delete-modal @table-state)]
    (do (GET "/jus/del-veza" {:params        {:parent level :child child}
                          :handler       #(init-veza)
                          :error-handler #(println "some error occured: " %)})
        (swap! count-veze update-in [level :total] dec)
        (swap! count-veze update-in [level :childs] #(remove #{child} %))
        )))
...
(defn set-path [JUSId level]
  (swap! table-state update-in [:path] (fn [] (into {} (take-while #(not= (next-level level) (key %)) (:path @table-state)))))
  (swap! table-state update-in [:path] #(assoc % (next-level level) JUSId))
  )

(defn reset-path []
  (swap! table-state assoc-in [:path] nil))
...
(def col-widths-types-naredbe-stare {:naslov  {:type :label :width "65%" :field "JUSopis" :action :click :function set-path}
...
(defn col-widths-types-jus [last-level alow-new-veza] (merge {:jusid  {:type :label :width "15%" :field "JUSId" :action false :label "JUS"}
                                                              :opis   {:type :label :width (if last-level "65%" "70%") :field "JUSopis" :action :click :function set-path :label "Naslov"}
                                                              :veze   {:type :label-tooltip :width "5%" :icon (icon-label "zmdi-attachment-alt" "Ukupan broj vezanih standarda") :field nil :action false :tooltip "Broj direktno vezanih JUS standarda: "}
                                                              :gotovo {:type :label-tooltip :width "5%" :icon (icon-label "zmdi-key" "Ukupan broj završenih vezanih standarda") :field nil :action false :tooltip "Završeno direktno vezanih JUS standarda: "}
                                                              :ok     {:type :check-box :width "5%" :icon (icon-label "zmdi-shield-check" "Završen unos vezanih standarda") :field "Locked" :disabled? last-level :action lock-jus}}
                                                             (if last-level {:brisi {:type     :row-button :gap "2px" :width "5%" :justify :end :icon (icon-label "zmdi-delete" "Brisanje reda")
                                                                                     :children [{:id               "r-b-1" :md-icon-name "zmdi zmdi-delete" :tooltip (if alow-new-veza "Obriši vezu!" "Ova veza je zaključana!")
                                                                                                 :tooltip-position :left-center :disabled? (disable-del nil) :action delete-veza-event}]}} nil)))
...
(defn rows-jus [data screen] (into {} (mapv (fn [x] (let [{:keys [JUSId JUSopis JUSgodina Locked]} x
                                                          count-veza (count-veza JUSId)]
                                                      {JUSId {:id       JUSId
...
(defn data-tables-level [level path screen]
  (let [alow-new-veza (alow-new-veza)
        last-level (= level (key (last path)))
        col-widths-types-jus (col-widths-types-jus last-level alow-new-veza)]
    ^{:key level}
    ;(println "l-l " level)
    [v-box
     ;:gap      "10px"
